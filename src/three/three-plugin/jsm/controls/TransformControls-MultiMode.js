...
function stopDomEventFromClassName(event) {
  return event.path.some((item) => {
    if (
      item.className &&
      String(item.className).indexOf("without-TransformControls-event") !== -1
    ) {
      return true;
    }
  });
...
function onPointerHover(event) {
  // 剔除不能操作的dom
  if (stopDomEventFromClassName(event)) {
    return;
  }
  if (!this.enabled) return;
...
class TransformControlsGizmo extends Object3D {
  constructor() {
    ...
    const pickerTranslate = {
      X: [
        [
          new Mesh(new CylinderGeometry(0.2, 0, 1, 4), matInvisible),
          [0.5, 0, 0],
          [0, 0, -Math.PI / 2],
        ],
        [
